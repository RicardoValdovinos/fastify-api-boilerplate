 ...
 */
const elasticsearch: FastifyPluginAsyncTypebox = async (instance: FastifyInstance, options: FastifyPluginOptions): Promise<void> => {
  const {
    ELASTIC_CLOUD_ID,
    ELASTIC_ADDRESS,
    ELASTIC_API_KEY
  ...
  const client = new Client({
    ...
    ...(options.elasticMock && { Connection: options.elasticMock.getConnection() })
  })
  ...
  instance.decorate('elastic', client)
  ...
  instance.decorate('indices', indices)
